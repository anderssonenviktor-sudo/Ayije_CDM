...
local function BuildLook(subPage, page)
    ...
    local desatHeader = UI.CreateHeader(rc, L["Icon Desaturation"])
    desatHeader:SetPoint("TOPLEFT", 0, yOff)
    yOff = yOff - 30

    local desatOutOfResourcesCheckbox = UI.CreateModernCheckbox(
        rc,
        L["Desaturate when out of resources instead of blue tint"],
        CDM.db.desaturateOutOfResources ~= false,
        function(checked)
            CDM.db.desaturateOutOfResources = checked
            API:Refresh("STYLE")
        end
    )

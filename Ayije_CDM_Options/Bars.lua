...
local function CreateBarsTab(page)
    ...
    local function ShowCustomBuffAddPanel(targetGroupIndex)
        pickerActiveGroupIndex = nil
        local _, rc = CreateRightScrollContent(500)
        local yOff = 0
        ...
        previewText:SetPoint("LEFT", sidInput, "RIGHT", 8, 0)
        previewText:SetText("")
        ...
        advAddBtn:SetScript("OnClick", function()
            ...
            if targetGroupIndex then
                local currentGroups = EnsureBarGroups()
        ...
        local height = math.abs(yOff) + 20
        if rc.SetHeight then rc:SetHeight(height) end
        FitRightPanel(height)
    end
    ...
    RefreshAll = function()
        ...
        if isViewingPlayer then
            ...
            for _, entry in ipairs(customOrder) do
                ungrouped[#ungrouped + 1] = { spellID = entry.spellID, isCustom = true }
            end
        end
        ...
        if #ungrouped == 0 then
            ...
            for _, entry in ipairs(ungrouped) do
                ...
                local isActive = entry.isCustom or not isViewingPlayer or IsSpellInActiveSet(activeSpellSet, sid)
                ...
                if entry.isCustom then
                    ...
                    row.removeBtn:SetScript("OnClick", function()
                        if selectedSpellID == sid and selectedSpellGroupIndex == nil then
                            selectedSpellID = nil
                            ClearRightPanel()
                        end
                        API:RemoveCustomBuffSpell(sid)
                        SaveAndRefresh()
                        RefreshLeftPanelIfNeeded()
                    end)
                end

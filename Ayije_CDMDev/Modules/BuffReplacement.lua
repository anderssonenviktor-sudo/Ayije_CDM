...
local function Matches(frame, spellID, cooldownID)
    ...
    for _, id in ipairs(CDM:GetSpellIDCandidates(frame)) do
        if id == spellID or (C_Spell.GetBaseSpell and C_Spell.GetBaseSpell(id) == spellID) then return true end
    end
    return false
end
        ...
        if Available(frame) then
            ...
            if CDM.IsSafeNumber(id) and CDM.IsSafeNumber(cooldownID) then
                options[#options + 1] = { value = cooldownID, spellID = id,
                    label = (C_Spell.GetSpellName(id) or tostring(id)) .. " (" .. cooldownID .. ")" }
            end
        end
    end
...
local function Restore(route)
    ...
    if route.source.Cooldown.SetCountdownFormatter then
        route.source.Cooldown:SetCountdownFormatter(CDM.CooldownFormatter.Get())
    end
end
        ...
        for source in viewer.itemFramePool:EnumerateActive() do
            if not bySource[source] and Available(source) and Matches(source, entry.replaceBuffSpellID, entry.replaceBuffCooldownID) then
                ...
                if not fd.buffSlotHooks then
                    ...
                    if source.Cooldown.SetCountdownFormatter then
                        hooksecurefunc(source.Cooldown, "SetCountdownFormatter", function(_, formatter)
                            local current = bySource[source]
                            if not current then return end
                            current.formatter = formatter
                            if current.formatterBusy then return end
                            local settings = CDM:ResolveCooldownSpellOverride(current.target)
                            local desired = CDM.CooldownFormatter.GetForSpell(settings) or CDM.CooldownFormatter.Get()
                            if formatter ~= desired then
                                current.formatterBusy = true
                                source.Cooldown:SetCountdownFormatter(desired)
                                current.formatterBusy = nil
                            end
                        end)
                    end
                end
...
function CDM:RefreshBuffReplacementSlots()
    ...
    for _, route in ipairs(routes) do
        ...
        if route.source.Cooldown.SetCountdownFormatter then
            local entry = self:ResolveCooldownSpellOverride(route.target)
            local formatter = self.CooldownFormatter.GetForSpell(entry) or self.CooldownFormatter.Get()
            if route.formatter ~= formatter then
                route.source.Cooldown:SetCountdownFormatter(formatter)
...
CDM:RegisterEvent("PLAYER_REGEN_ENABLED", function() CDM:QueueBuffReplacements() end)

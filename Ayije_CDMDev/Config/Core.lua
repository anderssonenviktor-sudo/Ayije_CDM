...
local function InitializeDB()
    if not Ayije_CDMDevDB then Ayije_CDMDevDB = {} end
    if not Ayije_CDMDevDB.profileKeys then Ayije_CDMDevDB.profileKeys = {} end
    if not Ayije_CDMDevDB.profiles then Ayije_CDMDevDB.profiles = {} end
    if not Ayije_CDMDevDB.global then Ayije_CDMDevDB.global = {} end
    Ayije_CDMDevDB.global.multiChargeSpells = nil
    if not Ayije_CDMDevDB.specProfiles then Ayije_CDMDevDB.specProfiles = {} end

    RunDatabaseMigrations()

    local charKey = UnitName("player") .. " - " .. GetRealmName()
    local defaultProfile = Ayije_CDMDevDB.global.defaultProfile or "Default"
    local profileName = Ayije_CDMDevDB.profileKeys[charKey] or defaultProfile
    Ayije_CDMDevDB.profileKeys[charKey] = profileName

    if not Ayije_CDMDevDB.profiles[profileName] then
        Ayije_CDMDevDB.profiles[profileName] = {}
    end

    local profile = Ayije_CDMDevDB.profiles[profileName]
    ...
    if not Ayije_CDMDevDB.global.cooldownViewerAutoEnabled then
        Ayije_CDMDevDB.global.cooldownViewerAutoEnabled = {}
    end
    if not Ayije_CDMDevDB.global.cooldownViewerAutoEnabled[charKey] then
        Ayije_CDMDevDB.global.cooldownViewerAutoEnabled[charKey] = true
...
function CDM:GetProfileList()
    local list = {}
    for name in pairs(Ayije_CDMDevDB.profiles) do
        list[#list + 1] = name
    end
    table.sort(list)
    return list
...
local function CommitActiveProfileReference(name)
    if not Ayije_CDMDevDB.profiles[name] then
        return false
    end
    Ayije_CDMDevDB.profileKeys[CDM.charKey] = name
    CDM.db = Ayije_CDMDevDB.profiles[name]
...
local function RebuildOptionsIfLoaded(targetTab)
    if not (C_AddOns and C_AddOns.IsAddOnLoaded and C_AddOns.IsAddOnLoaded("Ayije_CDMDev_Options")) then
        return
    end
...
function CDM:ApplyProfile(name, preparedProfileData, options)
    ...
    if not Ayije_CDMDevDB or not Ayije_CDMDevDB.profiles or not Ayije_CDMDevDB.profileKeys then
    ...
    if sourceProfileData == nil then
        sourceProfileData = Ayije_CDMDevDB.profiles[name]
    end
    if sourceProfileData == nil then
    ...
    Ayije_CDMDevDB.profiles[name] = commitProfileData
    CommitActiveProfileReference(name)

    if options.updateCurrentSpecProfile then
        local specData = Ayije_CDMDevDB.specProfiles
            and Ayije_CDMDevDB.specProfiles[self.charKey]
...
function CDM:NewProfile(name)
    ...
    if Ayije_CDMDevDB.profiles[name] then return false, "profile_exists" end
...
function CDM:CopyProfile(sourceName)
    if not Ayije_CDMDevDB.profiles[sourceName] then return false, "profile_not_found" end
    if sourceName == self.activeProfileName then return false, "source_is_active" end
    local source = Ayije_CDMDevDB.profiles[sourceName]
...
function CDM:DeleteProfile(name)
    ...
    if not Ayije_CDMDevDB.profiles[name] then return false, "profile_not_found" end
    local fallback = Ayije_CDMDevDB.global.defaultProfile or "Default"
    if fallback == name then fallback = "Default" end

    Ayije_CDMDevDB.profiles[name] = nil
    for charKey, profileName in pairs(Ayije_CDMDevDB.profileKeys) do
        if profileName == name then
            Ayije_CDMDevDB.profileKeys[charKey] = fallback
        end
    end
    if Ayije_CDMDevDB.specProfiles then
        for _, specData in pairs(Ayije_CDMDevDB.specProfiles) do
            for i = 1, 4 do
    ...
    if Ayije_CDMDevDB.global.defaultProfile == name then
        Ayije_CDMDevDB.global.defaultProfile = "Default"
    end
    return true
end
...
function CDM:RenameProfile(newName)
    ...
    if Ayije_CDMDevDB.profiles[newName] then return false, "profile_exists" end
    ...
    if not Ayije_CDMDevDB.profiles[oldName] then return false, "profile_not_found" end

    local renamedProfileData = Ayije_CDMDevDB.profiles[oldName]

    Ayije_CDMDevDB.profiles[newName] = Ayije_CDMDevDB.profiles[oldName]
    Ayije_CDMDevDB.profiles[oldName] = nil
    for charKey, profileName in pairs(Ayije_CDMDevDB.profileKeys) do
        if profileName == oldName then
            Ayije_CDMDevDB.profileKeys[charKey] = newName
        end
    end
    if Ayije_CDMDevDB.specProfiles then
        for _, specData in pairs(Ayije_CDMDevDB.specProfiles) do
    ...
    if Ayije_CDMDevDB.global.defaultProfile == oldName then
        Ayije_CDMDevDB.global.defaultProfile = newName
    end

    if self.db == renamedProfileData then
        self.db = Ayije_CDMDevDB.profiles[newName]
    end
...
function CDM:GetSpecProfileData()
    if not Ayije_CDMDevDB.specProfiles then return nil end
    return Ayije_CDMDevDB.specProfiles[self.charKey]
end
...
function CDM:SetSpecProfileEnabled(enabled)
    if not Ayije_CDMDevDB.specProfiles then Ayije_CDMDevDB.specProfiles = {} end
    local data = Ayije_CDMDevDB.specProfiles[self.charKey]
    if enabled then
        if not data then
            data = {}
            Ayije_CDMDevDB.specProfiles[self.charKey] = data
        end
        data.enabled = true
...
function CDM:CheckSpecProfileSwitch(specIndex)
    ...
    if not Ayije_CDMDevDB.profiles[targetProfile] then return end
...
local function OpenConfigNow(targetTab)
    if not C_AddOns.IsAddOnLoaded("Ayije_CDMDev_Options") then
        local loaded, reason = C_AddOns.LoadAddOn("Ayije_CDMDev_Options")
        if not loaded then

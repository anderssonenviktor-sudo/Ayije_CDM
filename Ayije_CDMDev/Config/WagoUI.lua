...
local API = {}
_G["Ayije_CDMDev_API"] = API

function API:ExportProfile(profileKey)
    if not Ayije_CDMDevDB or not Ayije_CDMDevDB.profiles then return nil end
    local profile = Ayije_CDMDevDB.profiles[profileKey]
...
function API:GetProfileKeys()
    local keys = {}
    if Ayije_CDMDevDB and Ayije_CDMDevDB.profiles then
...
function API:OpenConfig()
    ...
    if not C_AddOns.IsAddOnLoaded("Ayije_CDMDev_Options") then
        local loaded = C_AddOns.LoadAddOn("Ayije_CDMDev_Options")
...
function API:CloseConfig()
    local frame = _G["Ayije_CDMDevConfigFrame"]

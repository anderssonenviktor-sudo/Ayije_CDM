...
local function CollectBuffFramesInto(buffTbl, groupTbls, inEditMode, enforceHidden)
    ...
    if CB and CB.activeBuffs then
        ...
        local barGrouped = CDM.BarGroupSets and CDM.BarGroupSets.grouped
        for spellID, buffData in pairs(CB.activeBuffs) do
            local frame = buffData.frame
            -- A custom buff in a bar group renders as a bar, so its icon frame
            -- must not appear in the buff (icon) viewer. Hide it here so a stale
            -- shown state from before the group change can't leak through.
            if frame and barGrouped and barGrouped[spellID] then
                frame:Hide()
...
local function CollectBarFramesInto(barTbl, groupTbls, inEditMode)
    ...
    if CB and CB.activeBuffs then
        ...
        local grouped = barSets and barSets.grouped
        for spellID, buffData in pairs(CB.activeBuffs) do
            local frame = buffData.barFrame
            local groupIdx = grouped and grouped[spellID]
            if frame and groupIdx then
                -- Custom buff lives in a bar group: ensure its bar is shown and
                -- routed into the group container.
                frame:Show()
                frame.cdmBarGroupSpellID = spellID
                if not groupTbls[groupIdx] then groupTbls[groupIdx] = {} end
                groupTbls[groupIdx][#groupTbls[groupIdx] + 1] = frame
            elseif frame and frame:IsShown() then
                -- Not in a bar group (icon-only or moved out): its bar frame
                -- must not linger in the bar viewer.
                frame:Hide()
            end
        end
    end

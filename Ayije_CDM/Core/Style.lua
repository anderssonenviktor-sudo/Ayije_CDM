...
-- CDM tints frame.Icon with one of these fixed vertex colors after evaluating
-- C_Spell.IsSpellUsable in its RefreshIconColor pass. We classify the color
-- (a non-secret signal) rather than calling IsSpellUsable ourselves.
--   USABLE        = (1.0, 1.0, 1.0)
--   NOT_ENOUGH_MANA = (0.5, 0.5, 1.0)
--   NOT_USABLE    = (0.4, 0.4, 0.4)
--   NOT_IN_RANGE  = (0.64, 0.15, 0.15)
local function NearColor(x, y) return math_abs((x or 0) - (y or 0)) < 0.02 end

-- True when the vertex color CDM just wrote is its "not enough resources" tint.
-- CDM writes plain numeric color constants here, but guard against any secret
-- value so we never error or taint on comparison.
local function IsNotEnoughResourceTint(r, g, b)
    if not (IsSafeNumber(r) and IsSafeNumber(g) and IsSafeNumber(b)) then
        return false
    end
    return NearColor(r, 0.5) and NearColor(g, 0.5) and NearColor(b, 1.0)
end
    ...
    if desc and desc.hasOverride then
        if fullUpdate then
            ...
            if iconTex and not frameData.cdmDesatHooked then
                ...
                hooksecurefunc(iconTex, "SetDesaturated", function(icon)
                    local fd = GetFrameData(frame)
                    if fd.isProcessingOverride then return end

                    -- While resource-starved we own the desaturation; re-assert it
                    -- if CDM tries to re-saturate (it un-desaturates the resource
                    -- tint, which would otherwise undo our forced grey).
                    if fd.cdmResourceDesatActive and not fd.cdmResourceTintWrite then
            ...
            -- Force desaturation when CDM tints the icon for "not enough
            -- resources" (e.g. Evoker Essence). CDM signals this state by
            -- writing its NOT_ENOUGH_MANA vertex color to frame.Icon rather
            -- than desaturating, so we ride that write: classify the color and,
            -- when it's the resource tint, neutralize it to a clean grey.
            if iconTex and not frameData.cdmResourceTintHooked then
                frameData.cdmResourceTintHooked = true
                hooksecurefunc(iconTex, "SetVertexColor", function(icon, r, g, b)
                    local fd = GetFrameData(frame)
                    -- Skip our own writes to prevent infinite re-entry.
                    if fd.cdmResourceTintWrite then return end

                    -- Aura-override frames manage their own visuals.
                    if fd.cdmAuraOverrideActive or frame.cooldownUseAuraDisplayTime then
                        return
                    end
                    if FindAuraOverlayEntry(frame) then return end

                    -- Guard both the vertex and desaturation hooks against our writes.
                    fd.cdmResourceTintWrite = true
                    fd.isProcessingOverride = true
                    if styleCache.desaturateOutOfResources and IsNotEnoughResourceTint(r, g, b) then
                        -- Resource-starved: strip CDM's blue tint and desaturate.
                        icon:SetVertexColor(1, 1, 1)
                        icon:SetDesaturation(1)
                        fd.cdmResourceDesatActive = true
                    elseif fd.cdmResourceDesatActive then
                        -- Left the resource-starved state (or option disabled);
                        -- let the normal cooldown desaturation logic decide the look.
                        fd.cdmResourceDesatActive = false
                        WriteOverrideDesat(frame, GetCastSpellID(frame))
                    end
                    fd.isProcessingOverride = false
                    fd.cdmResourceTintWrite = false
                end)
            end

            -- Release a stuck forced desaturation when the option is turned off
            -- at runtime (otherwise the icon stays grey until CDM next re-tints).
            -- We only restore desaturation here; CDM re-applies its own resource
            -- tint on the next usability event. (Never call RefreshIconColor
            -- ourselves — it runs a secret IsSpellUsable test that taints.)
            if frameData.cdmResourceDesatActive and not styleCache.desaturateOutOfResources then
                frameData.cdmResourceDesatActive = false
                frameData.cdmResourceTintWrite = true
                frameData.isProcessingOverride = true
                WriteOverrideDesat(frame, GetCastSpellID(frame))
                frameData.isProcessingOverride = false
                frameData.cdmResourceTintWrite = false
            end

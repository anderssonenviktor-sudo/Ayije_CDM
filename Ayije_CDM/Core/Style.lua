...
-- CDM signals "not enough resources" by writing a NOT_ENOUGH_MANA vertex color
-- to frame.Icon (e.g. Evoker Essence) rather than desaturating. We classify the
-- color (a non-secret signal) rather than calling IsSpellUsable ourselves.
--   USABLE          = (1.0, 1.0, 1.0)
    ...
    if desc and desc.hasOverride then
        if fullUpdate then
            ...
            if not frame.cdmDesatHooked then
                ...
                hooksecurefunc(iconTex, "SetDesaturated", function(icon, desaturated)
                    ...
                    if frame.cdmResourceDesatActive and not frame.cdmResourceTintWrite then
                        frame.cdmInternalWrite = true
            ...
            if not frame.cdmResourceTintHooked then
                ...
                hooksecurefunc(iconTex, "SetVertexColor", function(icon, r, g, b)
                    ...
                    if styleCache.desaturateOutOfResources and IsNotEnoughResourceTint(r, g, b) then
                        ...
                        icon:SetDesaturation(1)
                        frame.cdmResourceDesatActive = true
                    elseif frame.cdmResourceDesatActive then
                        -- Left the resource-starved state (or option disabled);
                        -- let the normal cooldown desaturation logic decide the look.
                        frame.cdmResourceDesatActive = false
                        local entry = FindAuraOverlayEntry(frame)
                        local auraActive = (frame.cooldownUseAuraDisplayTime == true)
                        ApplyIconDesat(frame, entry, auraActive, GetCastSpellID(frame))
                    end
                    frame.cdmInternalWrite = false
                    frame.cdmResourceTintWrite = false
                end)
            end
            ...
            if frame.cdmResourceDesatActive and not styleCache.desaturateOutOfResources then
                frame.cdmResourceDesatActive = false
                frame.cdmResourceTintWrite = true
                local entry = FindAuraOverlayEntry(frame)
                local auraActive = (frame.cooldownUseAuraDisplayTime == true)

...
local function SetupDefaultProfileDropdown(dropdown)
    dropdown:SetDefaultText(Ayije_CDMDevDB.global.defaultProfile or "Default")
    dropdown:SetupMenu(function(_, rootDescription)
        local profiles = API:GetProfileList()
        local currentDefault = Ayije_CDMDevDB.global.defaultProfile or "Default"
        for _, name in ipairs(profiles) do
            rootDescription:CreateRadio(name, function()
                return currentDefault == name
            end, function()
                Ayije_CDMDevDB.global.defaultProfile = name
...
local function CreateProfilesTab(page, tabId)
    ...
    local newEditBox = UI.CreateCustomEditBox(newProfileRow)
    ...
    local createBtn = UI.CreateTextButton(newProfileRow)
    ...
    renameEditBox = UI.CreateCustomEditBox(renameRow)
    ...
    local renameBtn = UI.CreateTextButton(renameRow)
    ...
    local resetBtn = UI.CreateTextButton(manageRow)
